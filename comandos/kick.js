...
exports.run = (bot,message,args) => {
    ...
    if(!message.member.hasPermission("KICK_MEMBERS")) return message.reply(`Você não tem permissão para kickar um usuário`)
  ...
  if (message.mentions.users.first()) {
    ...
    message.channel.send(`**Quer realmente kickar esse usuário** **${membro}**?`).then(msg => {
        msg.react("✅")
    ...
    .setColor(bot.displayHexColor === '#000000' ? '#ffffff' : bot.displayHexColor)
      ...
      .setColor('#9c00ff')
      .setThumbnail(`${avatar}`)
      .setAuthor(`🛑 Usuário Kickado do servidor 🛑`,'https://cdn.discordapp.com/icons/476647620752703488/7e035d80a523369a0e510a3cc7edbb02.png?size=2048')
    ...
    message.channel.send(embed);
            ...
            canal.send(`**MEMBRO KICKADO COM SUCESSO**\n \nMembro: \`${membro.user.username}\`\nMotivo: ${motivo}`)
            membro.kick();*/
        })
    });
}
...
exports.help = {
    name: "kick",
    category: 'Função',
    description: "Apenas pode ser utilizado por um administrador para kickar usuários",
  }